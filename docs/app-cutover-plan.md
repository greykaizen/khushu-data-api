...
- **2026-09-21 (host session #2): Phase 4 CLOSED — fetcher-free construction shipped (orchestrator-v1.7.6, `d6e5d1a`).**
  - `KhushuContent`/`KhushuOrchestrator` accept `fetcher = null` as a first-class mode: islamic events resolve the `content` store per call (`SqlEventsSource` over `calendar_events`, 11 entries), the dua corpus already rides the resolver, and JSON-only domains (quran/catalog/curated/adhan streams, asma articles/streams, downloads/ledger, sunnahBooks) **fail fast on use** with a clear message instead of silently requiring a transport.
  - Khushu constructs with `fetcher = null` (`0f9cfa4`): the RemoteFetcher/CachingFetcher stack, raw-github base URL, and `content-v2026.09` pin are **deleted from the app**. All host-side retrieval already goes through `KhushuCorpus` pack repos.
  - **Proof (device)**: airplane mode + wiped `cacheDir/content` + cold start → zero errors, zero fail-fast triggers, live prayer times; 11/11 instrumented green on **both** the Pixel 10 emulator and the CPH2691 phone (real-network pack installs + FTS validation included).
  - Orchestrator: 173 tests green (was 167 at v1.7.4 → +6 across the two resolver/retirement rounds). JSON retirement tail remaining: mark the JSON `*Source` classes `@Deprecated` upstream and delete once no host passes a fetcher (quran-atlas glyph binaries stay transport-based for now — mushaf bundle loads still need a fetcher when used).
- **2026-09-21 (host session #3): Turso fallback drill — two latent production bugs fixed (`91e71cb`).**
  - The pack-absent → Turso routing had ZERO coverage and was silently broken: (1) `khushuSecret()` never read `local.properties` (Gradle doesn't auto-load it), so all six read-only tokens built into `BuildConfig` as `""` and `KhushuCorpus`'s remote map was all-null; (2) Turso's `libsql://` endpoints threw `MalformedURL` in `HttpURLConnection` — now normalized to `https://` for the v2/pipeline API.
  - Fix: explicit `local.properties` loading in `app/build.gradle.kts` (gitignored, `-P` still wins) + scheme normalization in `KhushuCorpus.remoteStore`.
  - New permanent coverage: `TursoRemoteFallbackTest` — delete the content pack → `KhushuCorpus.store` yields the remote store → identical SQL serves 491 duas; and the orchestrator aggregate (`resolver → orch.content.duas`) serves over Turso with the pack absent. Restores the pack afterwards.
  - 13/13 instrumented green on **both** Pixel 10 emulator and CPH2691 phone; lint + unit green.
  - **Atlas note (corrected)**: the Khushu app never calls `orch.mushaf`/atlas — mushaf mode reads `SqlMushafLayoutSource` over the quran-core pack. The atlas/glyph-texture surface is upstream-only (unused by hosts); no host-side gap exists.
- **2026-09-21 (host session #4): Quran browse list pack-served (`b519e85`) — the last hardcoded Quran surface closed.**
  - `QuranRepository.surahMetas()`: facts from `quran_surahs`, en names/meanings from `quran_surah_localizations`, SuraCon glyphs from the `quran_glyphs` config blob (chapter char + prefix). Sajdah stays the canonical-15 host constant (no pack table; browse filter only).
  - `QuranViewModel` swaps the generated `QuranStatic` table for pack data once the store resolves; the table remains the instant-first-frame + offline floor (canonical, just compile-time). `QuranBrowseListTest` pins entry-for-entry parity pack ↔ table (incl. glyph strings + the 15 sajdah surahs).
  - 14/14 instrumented green on emulator + CPH2691. **Remaining Quran gap (design-heavy, next increment)**: atlas-engine mushaf rendering — publish the uthmani/indopak atlas bundles as release assets (they exist in data-api `data/assets/atlas`, 4.7 MB, NOT in the pack manifest), a host download/unpack + `LocalFetcher` pass-through, and a Compose glyph-texture renderer. Current mushaf mode is font-composed text lines, not atlas textures.
