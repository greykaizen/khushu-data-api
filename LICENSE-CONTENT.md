...
| `inventory/fonts/` | Mushaf rendering fonts (`qpc/`, `kfqpc_v1/`) + icon fonts (`quran_icons/`: suracon, quran_common) + hadith text fonts (`sunnah/`: KFGQPC Uthman Taha Naskh, Noto Nastaliq Urdu, Noto Serif Bengali) | KFQPC / AlfaazPlus (GPLv3 family apps, fonts redistributed with them) / Google Noto (OFL) | Per-font: KFQPC free for non-commercial; Noto = OFL | ⚠️ KFQPC non-commercial; ✅ Noto OFL |
